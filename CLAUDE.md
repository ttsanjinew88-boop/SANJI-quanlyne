...
- **Nghi Ngờ**: tab mở cho **ADMIN + Tổ Trưởng** (`applyPerms` ~1478 + gate trong `renderSuspects`); đánh dấu/bỏ đại lý nghi ngờ vẫn chỉ ADMIN (`toggleSuspect`). Tô ô LƯU CLOUD: `BC.suspectMark` ghi `s.cellMarks` (["rowIdx|field"]) vào record `suspects`. Nút `#bcSusSheetBtn` (TT trở lên) → `BC.sendSuspectsToSheet`: POST JSON `{token,by,rows,colors,bolds}` (Content-Type text/plain tránh CORS preflight) tới `BC.GS_WEBAPP_URL` (**đang TRỐNG — chờ user deploy .gs rồi đưa URL để dán vào**), token `BC.GS_TOKEN='sanji-bc-2026'` phải trùng file .gs.
- `google_sheet_baocao.gs` (trong repo; user tự dán vào Apps Script của Sheet đích gid `1340440362`, hướng dẫn ở đầu file): `doPost` append dòng + setBackgrounds/FontWeights + `ensureHeader_`; `monthlyRotate` trigger 06:00 ngày 1 hằng tháng — nhân bản sheet thành "Báo cáo đại lý ngoài tháng M - YYYY" (M = tháng vừa kết thúc) rồi xóa dữ liệu chừa dòng 1; `setupTrigger` chạy tay 1 lần.
...
- **Gửi Nghi Ngờ → Google Sheet**: code XONG, CHỜ user (a) dán `google_sheet_baocao.gs` vào Apps Script của Sheet đích + deploy Web app (Execute as Me · Anyone) + chạy `setupTrigger` 1 lần, (b) gửi lại URL Web App để điền vào `BC.GS_WEBAPP_URL` trong `dashboard_v2.html`.
