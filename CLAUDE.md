...
- **Đổi tên nhân viên (chốt 01/08/2026)**: nút "✎ Đổi tên" mỗi dòng → `rosterRename(i)` (render.js). Khác thêm/ẩn (theo tháng): đổi tên áp cho **TẤT CẢ các tháng** — `renameMemberEverywhere(key,newName)` (data-boot.js) duyệt mọi report `type='roster'` (mọi tháng + bản `'all'` cũ, bỏ `CUR_MONTH`), sửa `name` của member trùng `key` rồi upsert lại; tháng đang mở do `saveRoster` ghi; tháng CHƯA có bản riêng tự lấy tên mới qua kế thừa. `key`/mã Excel/điểm số KHÔNG đổi (chỉ tên hiển thị). `search` (nhận diện FK trong note Excel) giữ nguyên — không đổi theo tên.
