...
// ⚠ supabase-js v2: auth.signOut() KHÔNG truyền scope = 'global' -> THU HỒI refresh token của
// tài khoản đó trên MỌI máy/MỌI tab. Chỉ cần 1 người bấm "Quay lại" ở màn 2FA hay đăng xuất là
// các máy khác đang mở dashboard bị văng ra ở lần F5/gia hạn token kế tiếp (nhìn giống "F5 nhiều
// lần là bị đăng xuất"). Luôn dùng hàm này -> chỉ xóa phiên TRÊN MÁY ĐANG THAO TÁC.
async function sbSignOutLocal(){
  try{await SB.client().auth.signOut({scope:'local'});}
  catch(e){try{await SB.client().auth.signOut();}catch(e2){}} // thư viện quá cũ không nhận scope
}
