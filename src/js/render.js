...
function onOvBlur(el){
  ...
  if(f==="wd"){
    // cho phép số thập phân (nửa ca): 22,5 hoặc 22.5 → 22.5; chỉ giữ chữ số + 1 dấu chấm
    val=val.replace(",",".").replace(/[^\d.]/g,"");
    const p=val.split(".");if(p.length>2)val=p[0]+"."+p.slice(1).join("");
    const num=val===""?null:Number(val);
    ovSet(fk,f,num!=null&&isFinite(num)?num:null);
  }
...
function rKoOverview(){
  ...
  if(lt){
    // LIỆT KÊ MỌI nhân viên đang hoạt động (kể cả chưa nhập hạn mức → hiện "—"),
    // LUÔN gom theo nhóm: FKVIP trước, FKONL sau (thêm người mới tự vào đúng cụm)
    const rows=FKVIP.concat(FKONL);
    lt.innerHTML='<thead><tr><th style="text-align:left">NHÂN VIÊN</th><th>Nhóm</th><th>Hạn mức duyệt đơn</th><th>Phê duyệt PCQ</th></tr></thead><tbody>'+
      ...
      }).join(''):'<tr><td colspan="4" style="color:var(--mu);padding:14px">Chưa có nhân viên nào — thêm trong Quản lý nhân viên</td></tr>')+'</tbody>';
  }
}
...
async function rosterRename(i){
  ...
  const curSearch=(m.search||m.key);
  const sv=prompt('Mã Excel (chuỗi nhận diện FK ở cột ghi chú file Excel) của "'+newName+'":',curSearch);
  if(sv===null)return;
...
async function limOpenTransfer(){
  ...
  const cnt=Object.keys(LIMITS).filter(fk=>LIMITS[fk]&&LIMITS[fk].limit).length;
  if(!cnt){alert('Tháng '+dispMonth(CUR_MONTH)+' chưa có hạn mức nào để chuyển.');return;}
  const box=document.getElementById('limCopyList');
  const desc=document.getElementById('limCopyDesc');
  desc.innerHTML='Chuyển <b style="color:var(--go)">'+cnt+'</b> hạn mức của tháng <b style="color:var(--cy)">'+dispMonth(CUR_MONTH)+'</b> sang tháng bạn chọn dưới đây:';
  box.innerHTML='<div style="color:var(--mu);font-size:.66rem;padding:8px">Đang tải danh sách tháng…</div>';
  document.getElementById('limCopyModal').classList.add('show');
  try{
    // Danh sách tháng đích = các tháng đã có report + vài tháng kế tiếp CUR_MONTH, bỏ chính tháng hiện tại
    const rows=await SB.listReports();
    const set=new Set((rows||[]).map(r=>r.month).filter(m=>/^\d{4}-\d{2}$/.test(m)));
    const [cy,cm]=CUR_MONTH.split('-').map(Number);
    for(let i=1;i<=6;i++){const d=new Date(cy,cm-1+i,1);set.add(d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0'));}
    set.delete(CUR_MONTH);
  ...
  }catch(e){console.error('limOpenTransfer',e);box.innerHTML='<div style="color:var(--re);font-size:.66rem;padding:8px">Lỗi tải danh sách: '+(e.message||e)+'</div>';}
}
function limCloseTransfer(){document.getElementById('limCopyModal').classList.remove('show');}
// Chuyển hạn mức (chỉ field limit) của tháng đang mở sang tháng đích; giữ PCQ sẵn có của tháng đích
async function limTransferTo(mk){
  if(!canEdit('ko')){alert('Bạn chỉ có quyền XEM.');return;}
  try{
    const src={};let n=0;
    for(const fk in LIMITS){const v=LIMITS[fk];if(v&&v.limit){src[fk]=v.limit;n++;}}
    if(!n){alert('Không có hạn mức để chuyển.');return;}
    const existing=await SB.loadReport('limits',mk);
    const had=existing&&Object.keys(existing).some(fk=>existing[fk]&&existing[fk].limit);
    if(had && !confirm('Tháng '+dispMonth(mk)+' ĐÃ có hạn mức. Ghi đè bằng '+n+' hạn mức từ tháng '+dispMonth(CUR_MONTH)+'?'))return;
    const out=existing?JSON.parse(JSON.stringify(existing)):{};
    for(const fk in src){if(!out[fk])out[fk]={};out[fk].limit=src[fk];}
    await SB.saveReport('limits',mk,out);
    logAction('Chuyển hạn mức duyệt','Chuyển '+n+' hạn mức từ tháng '+dispMonth(CUR_MONTH)+' sang tháng '+dispMonth(mk));
    setCloudStatus('Đã chuyển '+n+' hạn mức sang tháng '+dispMonth(mk)+' ✓');
    limCloseTransfer();
    // Nếu tháng đích chính là tháng đang mở (hiếm) thì reload lại
