...
function rKoOverview(){
  ...
  if(lt){
    // LUÔN gom theo nhóm: FKVIP trước, FKONL sau (thêm/sửa người mới cũng tự vào đúng cụm)
    const hasLim=fk=>LIMITS[fk]&&(LIMITS[fk].limit||LIMITS[fk].pcq);
    const rows=FK_KEYS.filter(fk=>FKVIP.includes(fk)&&hasLim(fk))
      .concat(FK_KEYS.filter(fk=>!FKVIP.includes(fk)&&hasLim(fk)));
