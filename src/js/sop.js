...
const SOP={
  ...
  async saveIndex(){
    ...
    catch(e){alert('Không lưu được mục lục: '+(e.message||e));}
  },
  async saveItem(){
    if(!SOP.cur)return;
    try{await SB.saveReport('sop_item',SOP.curId,SOP.cur);SOP.flash();}
    catch(e){alert('Không lưu được nội dung: '+(e.message||e));}
  },
  flash(){
  ...
  stepBarHtml(ed){
    ...
    if(!ss.length)return `<div class="sop-empty big">Chưa có bước nào.${ed?' Bấm ＋ Bước ở trên.':''}</div>`;
    let h='<div class="sop-steps">';
    ss.forEach((s,i)=>{
      if(i)h+='<div class="sop-sline"></div>';
      h+=`<div class="sop-sdot${i===SOP.curSub?' on':''}" onclick="SOP.goSub(${i})"><i>${i+1}</i><span data-noi18n>${hesc(s.name||'')}</span></div>`;
    });
    if(ed)h+=`<div class="sop-sline"></div><div class="sop-sdot sadd" onclick="SOP.addSub()"><i>+</i><span>Thêm bước</span></div>`;
    return h+'</div>';
  },
  stepBodyHtml(ed){
  ...
  // ---------- SỬA MỤC LỤC ----------
  async addGroup(){
    const n=prompt('Tên nhóm mới:');if(!n||!n.trim())return;
    SOP.groups().push({gid:SOP.uid('g'),name:n.trim(),items:[]});
    await SOP.saveIndex();SOP.renderRail();
  },
  async renameGroup(gi){
    const g=SOP.groups()[gi];if(!g)return;
    const n=prompt('Tên nhóm:',g.name);if(!n||!n.trim())return;
    g.name=n.trim();await SOP.saveIndex();SOP.renderRail();
  },
  async delGroup(gi){
    const g=SOP.groups()[gi];if(!g)return;
    if((g.items||[]).length&&!confirm(`Xoá nhóm "${g.name}" cùng ${g.items.length} mục bên trong?`))return;
    if(!(g.items||[]).length&&!confirm(`Xoá nhóm "${g.name}"?`))return;
  ...
  async addItem(){
    const gs=SOP.groups();
    if(!gs.length){alert('Hãy tạo Nhóm trước.');return;}
    const d=SOP.tabDef();
    const n=prompt(d.unit+' mới — nhập tên:');if(!n||!n.trim())return;
    let gi=0;
    if(gs.length>1){
      const pick=prompt('Thuộc nhóm nào? Nhập số:\n'+gs.map((g,i)=>(i+1)+'. '+g.name).join('\n'),'1');
      if(pick===null)return;
      gi=Math.max(0,Math.min(gs.length-1,(parseInt(pick,10)||1)-1));
    }
    const it={id:SOP.uid('i'),name:n.trim()};
    if(SOP.view==='ld'||SOP.view==='game')it.level=SOP.askLevel()||'mu';
  ...
  askLevel(){
    const L=SOP.view==='game'?SOP.GLEVELS:SOP.LEVELS;
    const v=prompt(`Mức độ:\n1. ${L.hi}\n2. ${L.go}\n3. ${L.mu}`,'3');
    return {1:'hi',2:'go',3:'mu'}[parseInt(v,10)]||null;
  },
  async renameItem(){
    const f=SOP.findItem(SOP.curId);if(!f)return;
    const n=prompt('Tên:',f.it.name);if(!n||!n.trim())return;
    f.it.name=n.trim();SOP.cur.name=n.trim();
    if(SOP.view==='ld'||SOP.view==='game'){const lv=SOP.askLevel();if(lv)f.it.level=lv;}
    await SOP.saveIndex();await SOP.saveItem();SOP.render();
  },
  async delItem(id){
    const f=SOP.findItem(id);if(!f)return;
    if(!confirm(`Xoá "${f.it.name}"? Không khôi phục được.`))return;
  ...
  async addSub(){
    ...
    const n=prompt(d.subName+' mới — nhập tên:');if(!n||!n.trim())return;
    const s={id:SOP.uid('s'),name:n.trim(),blocks:[]};
    if(d.subs==='acc'){s.level=SOP.askLevel()||'mu';s.code=(prompt('Mã game (bỏ trống nếu không có):','')||'').trim();s.from=(prompt('Áp dụng từ ngày (vd 01/08/2026):','')||'').trim();}
  ...
  async renameSub(i){
    ...
    const n=prompt('Tên:',s.name);if(!n||!n.trim())return;
    s.name=n.trim();SOP.touch();await SOP.saveItem();SOP.render();
  },
  async editGame(i){
    const s=SOP.cur.subs[i];if(!s)return;
    const c=prompt('Mã game:',s.code||'');if(c===null)return;
    const fr=prompt('Áp dụng từ ngày:',s.from||'');if(fr===null)return;
    const lv=SOP.askLevel();
    s.code=c.trim();s.from=fr.trim();if(lv)s.level=lv;
  ...
  async addBlock(scope,t){
    ...
    if(t==='text'){
      ...
      const cols=prompt('Bảng có mấy cột?','3');if(cols===null)return;
      const n=Math.max(1,Math.min(8,parseInt(cols,10)||3));
      const hd=prompt('Tiêu đề các cột, cách nhau dấu |','Cột 1 | Cột 2 | Cột 3');if(hd===null)return;
  ...
  async editBlock(scope,i){
    ...
    if(b.t==='text'){
      ...
      if(add){
        ...
        const r=prompt(`Nội dung ${n} ô, cách nhau dấu |`,'');if(r===null)return;
        const cells=r.split('|').map(s=>s.trim());while(cells.length<n)cells.push('');
        b.rows.push(cells.slice(0,n));
      }else{
        const h=prompt('Tiêu đề bảng:',b.h||'');if(h===null)return;b.h=h.trim();
      }
    }else if(b.t==='img'){
  ...
  async onFile(inp){
    ...
    for(let n=0;n<files.length;n++){
      ...
      }catch(e){alert('Tải ảnh thất bại: '+(e.message||e));break;}
    }
  ...
  async onTransFile(inp){
    ...
    let data;
    try{data=JSON.parse(await f.text());}catch(e){alert('File không đọc được: '+(e.message||e));return;}
    if(!data||!Array.isArray(data.items)){alert('File không đúng định dạng (thiếu items).');return;}
    ...
    alert(`Đã nhập bản dịch: ${nStr} chuỗi trong ${nItem} mục.`);
