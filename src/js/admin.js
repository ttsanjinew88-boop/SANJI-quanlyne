...
async function admLoadUsers(){
  ...
  try{
    ...
    list.innerHTML=`<table class="adm-tbl"><thead><tr><th>Tài khoản</th><th class="tc">Cấp bậc</th>${headPerms}${isAdmin?'<th class="tc">Admin</th>':''}<th class="tc">Khóa TK</th><th class="tc">Reset 2FA</th><th class="tc">Mật khẩu</th>${isAdmin?'<th class="tc">Xóa TK</th>':''}</tr></thead><tbody>`+
      data.map(u=>{
        const perms=u.perms||{};
        ...
        return `<tr><td style="font-weight:700;color:var(--tx)">${(u.username||'').toUpperCase()}${isSelf?' <span style="color:var(--pu2);font-size:.6rem">(bạn)</span>':''}</td>`+
          `<td class="tc">${roleCell}</td>${permCells}${adminCell}<td class="tc">${lockCell}</td><td class="tc">${resetCell}</td><td class="tc">${pwCell}</td>${isAdmin?`<td class="tc">${delCell}</td>`:''}</tr>`;
      }).join('')+'</tbody></table>';
  }catch(e){
...
// Xóa hẳn tài khoản (chỉ ADMIN) — dùng khi nhân viên nghỉ việc. KHÔNG hoàn tác được.
async function admDeleteUser(uid,uname){
  if(!CUR_PROFILE||!CUR_PROFILE.is_admin)return;
  const U=uname.toUpperCase();
  if(!confirm('XÓA HẲN tài khoản '+U+'?\n\n• Tài khoản này sẽ KHÔNG đăng nhập được nữa.\n• Hành động KHÔNG HOÀN TÁC được.\n\nLưu ý: điểm số / lịch sử đã ghi theo mã nhân viên (roster) VẪN GIỮ — đây chỉ xóa tài khoản đăng nhập.'))return;
  if(prompt('Để xác nhận, gõ đúng tên tài khoản: '+U)!==U){alert('Tên không khớp — đã hủy xóa.');return;}
  try{
    const{data:sess}=await SB.client().auth.getSession();
    const token=sess&&sess.session&&sess.session.access_token;
    if(!token){alert('Phiên hết hạn, đăng nhập lại');return;}
    const r=await fetch(SB_URL+'/functions/v1/super-function',{method:'POST',headers:{'Authorization':'Bearer '+token,'apikey':SB_KEY,'Content-Type':'application/json'},body:JSON.stringify({action:'delete_user',target:uid})});
    const res=await r.json();
    if(!res.ok)throw new Error(res.description||'Lỗi');
    logAction('Xóa tài khoản',U);
    alert('Đã xóa tài khoản '+U+' ✓');
    admLoadUsers();
  }catch(e){alert('Lỗi xóa tài khoản: '+(e.message||e));}
}
